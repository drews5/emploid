...
function TrustRing({ score, size=26 }) {
...
function Card({ app, onOpen, onDragStart, onDragEnd }) {
  ...
  return (
    <div
      className={`card ${app.hot ? 'hot' : ''} ${app.stall ? 'stall-card' : ''}`}
      draggable
    ...
    >
      ...
      <div className="card-top">
        ...
        <TrustRing score={app.trust} size={26}/>
      </div>
      ...
      <div className={`detail-panel ${app ? 'open' : ''}`}>
        <div className="detail-head">
          <div>
            ...
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              <TrustRing score={app.trust} size={32}/>
              <div>
                <div style={{fontSize:10,fontWeight:700,letterSpacing:'0.08em',textTransform:'uppercase',color: app.trust>=85?'var(--score-high)':app.trust>=70?'var(--score-mid)':'var(--score-low)'}}>
                  {app.trust>=85?'High Trust':app.trust>=70?'Review carefully':'Low Trust'}
                </div>
                <div style={{fontSize:11,color:'#5d7088',marginTop:1}}>Listing Trust Score</div>
              </div>
            </div>
          </div>
        ...
        <div className="detail-body">
          ...
          <div className="detail-section">
            ...
            <div className="detail-row" style={{borderBottom:'0'}}><div className="k">Days since</div><div className="v">{app.applied?daysBetween(app.applied, todayStr)+' days':'—'}</div></div>
          </div>
...
window.TrackerUI = { Icon, I, TrustRing, CompanyMark, Card, DetailPanel, todayStr, today, daysBetween, fmtDate };
